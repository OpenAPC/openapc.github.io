...
my.apc_new <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/master/data/snsf/snsf_openapc_update_20201125_enriched.csv"))
my.apc_new <- my.apc_new[!is.na(my.apc_new$institution),]
my.apc_new <- droplevels(my.apc_new)

```

The updated data set provided by the SNSF covers publication fees for `r format(nrow(my.apc_new), big.mark =",")` articles published in 2019 and 2020. Total expenditure amounts to `r sum(my.apc_new$euro)`€ and the average fee is `r sum(my.apc_new$euro)/nrow(my.apc_new)`€. The following table provides an overview:
...
```{r box_snsf_2020_11_26_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_snsf_2020_11_26_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
