...
The updated data set provided by the CSC covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published between 2018 and 2021 by 10 different institutions, replacing the previous CSC data. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The CSC also provided some BPC data for OA monographs (not shown here).
...
```{r box_csc_2022_01_13_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
