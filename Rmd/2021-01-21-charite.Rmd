...
my.apc_new <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/v4.8.3-2-0/data/charite/2019_openapc_charite_wiley-deal_gold_enriched.csv"))
my.apc_new <- droplevels(my.apc_new)

```
The new data set covers publication fees for `r format(nrow(my.apc_new), big.mark =",")` Gold OA articles published under DEAL. Total expenditure amounts to `r sum(my.apc_new$euro)`€ and the average fee is `r sum(my.apc_new$euro)/nrow(my.apc_new)`€.
...
```{r tree_charite_2021_01_21_full}
...
```{r box_charite_2021_01_21_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_charite_2021_01_21_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
