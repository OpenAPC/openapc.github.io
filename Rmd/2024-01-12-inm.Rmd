...
my.apc <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/v4.112.2-0-0/data/apc_de.csv"))
my.apc <- my.apc[200715:200717,]
...
The data set provided by INM covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r box_inm_2024_01_12_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
