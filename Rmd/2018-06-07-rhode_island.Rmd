...
The initial dataset provided by the URI covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published from 2014 to 2018. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r tree_rhode_island_2018_06_07_full}
...
```{r box_rhode_island_2018_06_07_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
