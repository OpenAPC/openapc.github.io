...
my.apc <- my.apc[my.apc$institution == "Consiglio Nazionale Delle Ricerche (CNR)",]
my.apc <- droplevels(my.apc)

```

The initial data set provided by the National Research Council of Italy covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r box_cnr_2022_12_02_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_cnr_2022_12_02_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
