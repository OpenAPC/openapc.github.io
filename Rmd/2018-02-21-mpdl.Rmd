...
my.apc_various_2017 <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/master/data/mpg/mpg_diff_articles_2017_enriched.csv"))
...
The new dataset covers publication fees for `r format(nrow(my.apc_various_2017), big.mark =",")` articles published by MPG researchers with the aforementioned publishers. Total expenditure amounts to `r sum(my.apc_various_2017$euro)`€ and the average fee is `r sum(my.apc_various_2017$euro)/nrow(my.apc_various_2017)`€.

```{r}
d_frame = data.frame(table(my.apc_various_2017$journal_full_title,  dnn="Journal"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Journal)), ]                                           
                                    
my.apc_various_2017$journal_full_title <- factor(my.apc_various_2017$journal_full_title, levels =  d_frame$Journal)

df.summary <-cbind(tapply(my.apc_various_2017$euro, my.apc_various_2017$journal_full_title, length),
             tapply(my.apc_various_2017$euro, my.apc_various_2017$journal_full_title, sum),
             tapply(my.apc_various_2017$euro, my.apc_various_2017$journal_full_title, mean))
...
```{r box_mpdl_2018_02_21_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_mpdl_2018_02_21_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
