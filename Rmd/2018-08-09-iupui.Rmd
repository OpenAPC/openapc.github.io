...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution == "Indiana University - Purdue University Indianapolis (IUPUI)",]
my.apc <- droplevels(my.apc)
```

The initial dataset provided by the IUPUI covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published from 2013 to 2018. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r box_iupui_2018_08_09_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
