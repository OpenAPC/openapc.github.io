...
my.apc_new <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/master/data/unibern/Publikationsfonds_2021_Jan-Jun_enriched.csv"))
my.apc_new <- my.apc_new[my.apc_new$institution != "",]
my.apc_new <- droplevels(my.apc_new)

```

The new data set covers publication fees for `r format(nrow(my.apc_new), big.mark =",")` articles published in the first half 2021. Total expenditure amounts to `r sum(my.apc_new$euro)`€ and the average fee is `r sum(my.apc_new$euro)/nrow(my.apc_new)`€.
...
```{r box_bern_2021_08_10_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_bern_2021_08_10_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
