...
The new dataset covers publication fees for `r format(nrow(my.apc_plos_16), big.mark =",")` articles published by MPG researchers in PLoS journals. Total expenditure amounts to `r sum(my.apc_plos_16$euro)`€ and the average fee is `r sum(my.apc_plos_16$euro)/nrow(my.apc_plos_16)`€.

```{r}
d_frame = data.frame(table(my.apc_plos_16$journal_full_title,  dnn="Journal"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Journal)), ]                                           
                                    
my.apc_plos_16$journal_full_title <- factor(my.apc_plos_16$journal_full_title, levels =  d_frame$Journal)

df.summary <-cbind(tapply(my.apc_plos_16$euro, my.apc_plos_16$journal_full_title, length),
             tapply(my.apc_plos_16$euro, my.apc_plos_16$journal_full_title, sum),
             tapply(my.apc_plos_16$euro, my.apc_plos_16$journal_full_title, mean))
...
```{r box_mpdl_2017_08_09_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_mpdl_2017_08_09_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
