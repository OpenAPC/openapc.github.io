...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution == "Fraunhofer Publishing Fund",]
my.apc <- droplevels(my.apc)
```

The initial dataset provided by the Fraunhofer Society covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published in 2017. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r tree_fraunhofer_2018_05_23_full}
...
```{r box_fraunhofer_2018_05_23_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
