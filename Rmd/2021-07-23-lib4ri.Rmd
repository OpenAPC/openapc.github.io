...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution %in% c("Empa - Swiss Federal Laboratories for Materials Science and Technology", "Eawag - Swiss Federal Institute of Aquatic Science and Technology"),]
my.apc <- my.apc[my.apc$period == 2020,]
...
The new data set covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published at Empa and Eawag in 2020. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The following tables and plots provide more details.
...
```{r tree_lib4ri_empa_2021_07_23_full}
...
```{r box_lib4ri_empa_2021_07_23_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc_empa$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
...
```{r tree_lib4ri_eawag_2021_07_23_full}
...
```{r box_lib4ri_eawag_2021_07_23_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc_eawag$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
publishers = as.character(d_frame$Publisher[d_frame$Freq > 0])
