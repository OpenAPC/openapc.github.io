...
The new data set provided by the Qatar National Library covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published between 2017 and 2021, this includes 30 articles which had been reported previously. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r box_qnl_2021_07_09_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_qnl_2021_07_09_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
