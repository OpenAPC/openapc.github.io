...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution == "Université Clermont Auvergne",]
...
The new data set covers publication fees for `r format(nrow(my.apc_new), big.mark =",")` articles published in 2018. Total expenditure amounts to `r sum(my.apc_new$euro)`€ and the average fee is `r sum(my.apc_new$euro)/nrow(my.apc_new)`€.
...
```{r tree_clermont_auvergne_2020_01_08_full}
...
```{r box_clermont_auvergne_2020_01_08_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_clermont_auvergne_2020_01_08_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
