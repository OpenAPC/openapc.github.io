...
my.apc <- download_apc(c("https://raw.githubusercontent.com/OpenAPC/openapc-de/v4.121.9-7-6/data/csc/journals_enriched.csv"))
...
The updated data set provided by the CSC covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published between 2017 and 2024, replacing the previous CSC data. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r box_csc_2024_03_01_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
