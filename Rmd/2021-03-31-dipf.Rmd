...
The initial dataset provided by the DIPF covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published in 2019 and 2020, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The DIPF also provided BPC data for 3 OA monographs (not shown here).
...
```{r box_dipf_2021_03_31_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
