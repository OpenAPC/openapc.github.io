...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution == "University of Applied Sciences Mainz",]
my.apc <- droplevels(my.apc)

```

The initial data set provided by the HS Mainz covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r tree_hs_mainz_2025_01_22_full}
...
```{r box_hs_mainz_2025_01_22_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
