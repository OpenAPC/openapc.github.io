...
The initial data set provided by Bauhaus-Universität Weimar covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published from 2019 to 2021, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The contribution also contained two records for Open Access monographs which are not displayed here.
...
```{r box_weimar_2022_03_16_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_weimar_2022_03_16_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
