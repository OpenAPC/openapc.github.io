...
The initial data set provided by the KU Eichstätt-Ingolstadt covers publication fees for `r format(nrow(my.apc), big.mark =",")` journal articles, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The contribution also contained BPC data for one OA monograph which is not listed here.
...
```{r box_kuei_2022_05_31_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_kuei_2022_05_31_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
