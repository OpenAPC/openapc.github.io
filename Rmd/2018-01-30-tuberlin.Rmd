...
The first dataset provided by the TU covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r tree_tuberlin_2018_01_30_full}
...
```{r box_tuberlin_2018_01_30_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_tuberlin_2018_01_30_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
