...
The initial data set provided by the HAW Hamburg covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles published in 2020 and 2021, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The contribution also provided cost data on articles published in **hybrid** journals under the DEAL agreement with Springer Nature and Wiley in 2020 which are not included in this list.
...
```{r tree_haw_hamburg_2022_05_11_full}
...
```{r box_haw_hamburg_2022_05_11_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_haw_hamburg_2022_05_11_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
