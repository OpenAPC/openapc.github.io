...
The initial data covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The following tables and plots provide an overview of the new data set:
...
```{r tree_saarland_2020_07_07_full}
...
```{r box_saarland_2020_07_07_year_full, echo=FALSE, message = FALSE}
require(ggplot2)
...
```{r box_saarland_2020_07_07_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
