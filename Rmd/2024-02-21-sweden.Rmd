...
The new data set provided by the National Library covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles. Total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€. The contribution also contained BPC data for one OA monograph which is not listed here.
...
```{r tree_natlibswe_2024_02_21_full}
...
```{r box_natlibswe_2024_02_21_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
