...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution == "Fulda University of Applied Sciences",]
my.apc <- droplevels(my.apc)

```

The initial data set provided by Fulda covers publication fees for `r format(nrow(my.apc), big.mark =",")` articles, total expenditure amounts to `r sum(my.apc$euro)`€ and the average fee is `r sum(my.apc$euro)/nrow(my.apc)`€.
...
```{r tree_hs_fulda_2025_02_19_full}
...
```{r box_hs_fulda_2025_02_19_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
