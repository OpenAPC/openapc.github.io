...
my.apc <- download_apc()
my.apc <- my.apc[my.apc$institution %in% c("Université d'Avignon et des Pays de Vaucluse", "Université de Caen Normandie", "Université de France-Comté", "Université Paris Diderot (Paris VII)", "Université de Strasbourg"),]
...
```{r tree_couperin_2018_10_31_full}
...
```{r box_couperin_2018_10_31_publisher_full, echo = FALSE, message = FALSE}
require(ggplot2)
d_frame = data.frame(table(my.apc$publisher,  dnn="Publisher"))                 
d_frame = d_frame[with(d_frame, order(-Freq, Publisher)), ]
